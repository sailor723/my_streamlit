    ...
    if selected_layers:
        st.pydeck_chart(pdk.Deck(
            ...
            tooltip={
                ...
                'html': '<b>中心名称:</b> {中心名称1} 器械数量:{elevationValue} <b>  \
    ...
    if post_id not in df_post['运单号'].tolist():
        ...
        for device_id in device_id_list:
                ...
                if device_id in df.index:
                    ...
                    if (target_site != df.loc[device_id,'中心名称']) & (target_site in df['中心名称'].tolist()):
                        ...
                        if df.loc[device_id, '状态'] == '可用':
                            ...
                            series = pd.Series({"运单号":post_id,"序号":device_id,"始发库":df.loc[device_id,
                                                '中心名称'],"目标中心":target_site,"发送日期":post_date,"接收日期":'NA',
                                               's_lon': df.loc[device_id]['lon'],
                                               's_lat': df.loc[device_id]['lat'],
                                               'r_lon': df[df['中心名称'] == target_site]['lon'].unique()[0],
                                               'r_lat': df[df['中心名称'] == target_site]['lat'].unique()[0],
                                                })


                            df_post = df_post.append(series,ignore_index=True)
                            df.loc[device_id, 'lon'] = df[df['中心名称'] == target_site]['lon'].unique()[0]
                            update_excel(device_id, column_name = 'lon', sheet_name = sheetMain)#  update 状态

                            df.loc[device_id, 'lat'] = df[df['中心名称'] == target_site]['lat'].unique()[0]
                            update_excel(device_id, column_name = 'lat', sheet_name = sheetMain)#  update 状态

                            df.loc[device_id, '中心名称'] = target_site
                            ...
                            update_excel(device_id, column_name = '中心名称', sheet_name = sheetMain)#  update 状态
    ...
    if post_id in df_post['运单号'].tolist():
        ...
        for i in df_post[df_post['运单号'] == post_id].index.tolist():
            ...
            if device_id in df.index:
                ...
                if target_site == df.loc[device_id,"中心名称"]:
...
def surgery_plan (device_id,target_site,patient_id,target_date,sheet_name):
    ...
    if device_id in df.index:
        ...
        if target_site == df.loc[device_id,"中心名称"]:
            ...
            print('中心名称错误，请确认 %s  %s :' %(device_id,df.loc[device_id, '中心名称']))
            return('2')
    else:
        print('设备号错误，请确认: ', device_id)
        return('1')
...
col1, col3, col2 = st.beta_columns([2, 2, 2])

with col1:
    st.subheader("器械发送管理")
    target_site = st.selectbox(
    "请选择收货中心",
    df['中心名称'].unique().tolist())
    ...
    if original_site != target_site:
        device_list = st.multiselect( "请选择设备",
                            df[(df['中心名称'] == original_site) & (df['状态'] == '可用') ] ['序号'].tolist())


        carrier_number = st.text_input('Please input carrier number')
        st.write('The current number is ', carrier_number)

        carrier_date = st.date_input(
            "请输入发货日期",
            datetime.date(2019, 7, 6))

        st.write('发货日期:', carrier_date)
...
with col4:
    ...
    surgery_device = st.selectbox(
        "请选择手术器械序号",
         df[(df['中心名称'] == surgery_site) & (df['状态'] == '可用')] ['序号'].tolist())
...
with col6:
    ...
    surgery_site = st.selectbox(
    "请选择研究中心",
    df[df['状态'] == 'in plan']['中心名称'].unique().tolist(),
    key = 'surgey_site_complete')

    surgery_device = st.selectbox(
        "请选择手术器械序号",
         df[(df['中心名称'] == surgery_site) & (df['状态'] == 'in plan')] ['序号'].tolist(),
...
with st.beta_expander("计划和完成的器械"):
    st.table( df[(df['状态'] == 'in plan') | (df['状态'] == '完成')][['中心名称','器械名称','受试者编号','使用日期','状态']])
