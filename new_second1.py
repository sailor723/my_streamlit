...
st.header('脑血管疾病诊疗高端介入医学产品')
st.subheader('器械中心分布')

excel_file = 'C:\\Users\\weiping\\Documents\\eclincloud2020\\training\\Python_dev\\code\\streamlit\\my_test\\器械模拟20210623 - 副本.xls'

image = Image.open('C:\\Users\\weiping\\Documents\\eclincloud2020\\Customers\\加奇生物\\images\\logo.jpg')
...
df_xx = df.sort_values(['site_name','状态'],ascending=True)
bar_chart = px.bar(df_xx,
                  x = 'site_name',
                  y = '状态',
                #   text = 'model',
                  color='状态',
...
df_y = df.sort_values(['site_name','状态'],ascending=True)
fig = px.histogram(df_y, x="site_name",color='状态')
st.plotly_chart(fig)

df_x = df.sort_values(['model_spec','状态'],ascending=True)
fig = px.histogram(df_x, x="model_name",color='状态')
st.plotly_chart(fig)
