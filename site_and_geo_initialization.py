...
#___________________read into sheet, convert to dataframe___________________________________________
...
for sheet_num in range(0, len(wb.sheets)-4):
    ...
    new_column = ['中心名称'] + df_read.columns.tolist() 
...
df[['site_num','site_name']] = df['中心名称'].str.split(' ', expand=True)
df['序号'] = df['序号'].astype(int)
df_columns = ['site_num','site_name',
 '序号',
 '器械名称',
 '规格型号',
 '序列号',
 ...
 '受试者编号']
df = df[df_columns]
df = df.sort_values(by=['site_num','序号'],ascending=[True,True])
...
#_________________________________convert time to datatime format and create data_range__________
...
df['model_spec'] = item_model

df_columns = ['site_num','site_name', '序号', '器械名称', '规格型号',  'model_name', 'model_spec','序列号', '批号', '效期', '接收日期',
       '使用日期', '状态','受试者编号','date_range']
df = df[df_columns]


# ——————————————————————————————————to have num, date range——————————————————————————————————————
df_geo = pd.DataFrame(df.groupby('site_name').size(),columns=['site_dev_num'])   
df_geo['date_range_mean'] = df.groupby('site_name')['date_range'].mean().astype(int)
...
df_4 = df_4.sort_values(by=['site_num','序号'],ascending=[True,True])
site_last = 'x'
...
df_5 = df[df['状态'] == '完成'].sample(frac=0.3,random_state=1234)
...
#————————————————————————————————————————to write into whole datasheet————————————————————————————
sheet1 = wb.sheets('Whole')   # create Working sheet
#sheet1.range('E1').api.EntireColumn.NumberFormat = '@'
sheet1.range('A1').options(index=True,numbers = str).value = df.iloc[:] # write to Working sheet, number must be int
...
df = df.join(df_geo, on ='site_name')
...
df_whole_geo = df_whole_geo.sort_values(by=['site_name','序号'],ascending=[True,True])
df_whole_geo.set_index(['site_name', '序号'],inplace=True)
